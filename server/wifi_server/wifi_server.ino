...
void handleRoot() {
  toggle=!toggle;               //Toggling Led Variable
    digitalWrite(BUILTIN_LED,toggle);     //Toggle Led
...
void setup() {
  ...
  pinMode(BUILTIN_LED, OUTPUT);         //Led/Solenoid at pin 2
...
void handleColor()
{
  ...
  int g = random(0,255);
  int b = random(0,255);
  String s = "<MESSAGE>color|"+String(r)+"|"+String(g)+"|"+String(b)+"|]";
  server.send(200,"color/html",s);      //Reply to the client
}
